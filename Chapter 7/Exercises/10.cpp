...
// reads input from cin to parse into a Token
{
	...
	case '5': case '6': case '7': case '8': case '9':
	{
		...
		int i = narrow_cast<int>(val); // convert to int and check for truncation
		return Token(number, i);	// return a number Token
	}
	default:
		if (isspace(ch)) {
...
public:
	...
	int value;
	bool constant;
	Variable(string n, int v, bool b) :name(n), value(v), constant(b) { }
...
public:
	...
	int declare(string var, int val, bool b);
private:
...
int primary();	// declare primary() so that exp() can see it
...
int expression(); // declare expression() so other functions can see it
...
//int define_name(string var, int val)
//// add (var, val) to var_table
//{
//	if (is_declared(var)) error(var, " already declared");
...
//int define_name(string var, int val, bool b)
//// add (var, val) to var_table
//{
//	if (is_declared(var)) error(var, " already declared");
//	var_table.push_back(Variable{ var, val, b });
...
int declaration(bool constant)
{
	...
	if (t2.kind != '=') error("= missing in declaration of ", name); // is it an
																	 // '=' Token?
	int d = expression(); // get the value assigned to the Variable
